...
from ..models import EventAttendance, Event, User
...
class EventAttendanceCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if event.end_time < timezone.now():
            raise serializers.ValidationError({
                'event': "Cannot RSVP to past event"
            })
...
class EventStatisticsSerializer(serializers.Serializer):
    ...
    class Meta:
        fields = [
            'total_attendees', 'going_count', 'maybe_count',
            'declined_count', 'is_full', 'remaining_spots',
            'days_until_event', 'duration_hours', 'organizer', 'group'
        ]
...
class UserAttendanceStatisticsSerializer(serializers.Serializer):
    ...
    class Meta:
        fields = [
            'total_rsvps', 'status_breakdown', 'upcoming_events',
            'past_events_attended', 'events_organized', 'attendance_rate'
        ]