...
class EventAttendanceService:
    ...
    def get_user_events(
        ...
        # Extract events
        events = [attendance.event for attendance in attendances.order_by('event__start_time')]
    ...
    def get_user_attendance_statistics(user: User) -> Dict[str, Any]:
        ...
        return {
            ...
            'attendance_rate': (past / organized_count * 100) if organized_count > 0 else 0
        }
    
    @staticmethod
    def get_mutual_attendees(
        event: Event,
        user: User
    ) -> List[Dict[str, Any]]:
    ...
    def get_attendance_trend(
        ...
        # Group by hour
        trend = EventAttendance.objects.filter(
            event=event,
            joined_at__gte=time_threshold
        ).annotate(
            hour=TruncHour('joined_at')
