...
class UserSearchSerializer(serializers.Serializer):
    ...
    include_follow_status = serializers.BooleanField(
        required=False, default=False, help_text="Include follow relationship status"
    )

    def search(self) -> List[User]:
        """Perform user search"""
        query = self.validated_data["query"]
        limit = self.validated_data.get("limit", 100)
        only_active = self.validated_data.get("only_active", None)
...
class SearchResultSerializer(UserListSerializer):
    ...
    def get_mutual_follows(self, obj) -> int:
        ...
        if request and request.user.is_authenticated:
            ...
            obj_following = set(
                UserFollow.objects.filter(follower=obj).values_list(
                    "following_id", flat=True
                )
            )
