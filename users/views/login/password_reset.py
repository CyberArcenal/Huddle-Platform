        ...
        examples=[
            ...
            OpenApiExample(
                ...
                value={
                    ...
                    "changed_at": "2025-03-08T12:34:56Z",
                },
                response_only=True,
            ),
        ],
...
class PasswordStrengthCheckView(APIView):
    ...
    def _get_password_suggestions(self, password):
        ...
        if any(
            ...
            and int(password[i : i + 3]) in range(100, 1000)
            and int(password[i : i + 3]) % 111 == 0
...
class PasswordHistoryView(APIView):
    ...
    def get(self, request):
        ...
        password_events = SecurityLog.objects.filter(
            user=user, event_type__in=["password_changed", "password_change_failed"]
        ).order_by("-created_at")

        paginated_events = self.pagination_class().paginate_queryset(
            password_events, request
        )
        events_data = []
        ...
        return Response({"total_events": len(events_data), "events": events_data})
