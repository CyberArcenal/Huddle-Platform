...
@extend_schema_view(
    get=extend_schema(
        parameters=[
            OpenApiParameter(name='id', type=int, location=OpenApiParameter.PATH, description='Checkpoint ID (optional)'),
            OpenApiParameter(name='is_used', type=bool, location=OpenApiParameter.QUERY, description='Filter by used status'),
            OpenApiParameter(name='is_valid', type=bool, location=OpenApiParameter.QUERY, description='Filter by validity'),
            OpenApiParameter(name='search', type=str, location=OpenApiParameter.QUERY, description='Search in username, email, token'),
        ],
...
class LoginCheckpointCRUD(APIView):
    ...
    def get(self, request, id=None):
        ...
        try:
            if id is not None:
                ...
                serializer = LoginCheckpointSerializer(checkpoint, context={"request": request})
            ...
            if is_valid:
                ...
                if is_valid.lower() == "true":
                    ...
                    qs = qs.filter(models.Q(is_used=True) | models.Q(expires_at__lte=timezone.now()))
            if search:
                qs = qs.filter(
            ...
            serializer = LoginCheckpointSerializer(page, many=True, context={"request": request})
            ...
            return Response({"detail": "Login checkpoint not found."}, status=status.HTTP_404_NOT_FOUND)
    ...
    def post(self, request):
        ...
        serializer = LoginCheckpointSerializer(data=request.data, context={"request": request})
