...
class UserActivityService:
    ...
    def get_user_activities(
        user: User, action: Optional[str] = None, limit: int = 50, offset: int = 0
    ) -> List[UserActivity]:
    ...
    def get_recent_activities(
        limit: int = 100, action: Optional[str] = None, user: Optional[User] = None
    ) -> List[UserActivity]:
    ...
    @staticmethod
    def get_following_activities(user: User, limit: int = 50) -> List[UserActivity]:
        ...
        return list(
            UserActivity.objects.filter(user__in=following_users).order_by(
                "-timestamp"
            )[:limit]
        )
    ...
    def cleanup_old_activities(days: int = 365) -> int:
        ...
        old_activities = UserActivity.objects.filter(timestamp__lt=time_threshold)
