...
from ..serializers.base import (
    ReportedContentSerializer, ReportContentInputSerializer,
    ReportFilterSerializer, ReportStatusUpdateSerializer,
    ResolveReportInputSerializer, ReportStatisticsSerializer,
...
class ReportListView(APIView):
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(name='content_type', type=str, description='Filter by content type', required=False),
            OpenApiParameter(name='reporter_id', type=int, description='Filter by reporter ID', required=False),
            ...
            OpenApiParameter(name='unresolved_only', type=bool, description='Show only unresolved reports', required=False),
            OpenApiParameter(name='page', type=int, description='Page number', required=False),
            OpenApiParameter(name='page_size', type=int, description='Results per page', required=False),
        ],
        responses={200: ReportedContentSerializer(many=True)},
...
class ReportDismissView(APIView):
    ...
    def post(self, request, report_id):
        ...
        reason = request.data.get('reason', 'Report dismissed')
        try:
            ...
            serializer = ReportedContentSerializer(dismissed_report)
            return Response(serializer.data)
        except ValidationError as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class ReportUrgentView(APIView):
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter(name='threshold', type=int, description='Minimum number of reports to be considered urgent', required=False),
...
class ReportUserHistoryView(APIView):
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter(name='as_reporter', type=bool, description='Include reports made by the user', required=False),
            OpenApiParameter(name='page', type=int, description='Page number', required=False),
            OpenApiParameter(name='page_size', type=int, description='Results per page', required=False),
        ],
        responses={200: ReportedContentSerializer(many=True)},
...
class ReportCleanupView(APIView):
    ...
    @extend_schema(
        request={'application/json': {'days_to_keep': 180}},
        responses={200: {'type': 'object', 'properties': {'message': {'type': 'string'}}}},
        description="Delete old resolved/dismissed reports."
    )
    def post(self, request):
        days_to_keep = int(request.data.get('days_to_keep', 180))
