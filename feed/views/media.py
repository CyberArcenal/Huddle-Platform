...
from feed.serializers.media import MediaDisplaySerializer, MediaCreateSerializer, MediaMinimalSerializer
...
class MediaListView(APIView):
    ...
    def get(self, request):
        ...
        group_content_type = request.query_params.get('group_content_type')  # 'post' or 'reel'
        ...
        if post_id:
            try:
                ...
                return Response({"error": "Post content type not found"}, status=status.HTTP_400_BAD_REQUEST)
        elif reel_id:
            try:
                ...
                return Response({"error": "Reel content type not found"}, status=status.HTTP_400_BAD_REQUEST)
        elif group_id:
            ...
            if group_content_type == 'post':
                queryset = queryset.filter(content_type=post_ct, object_id__in=Subquery(post_ids_sub))
            elif group_content_type == 'reel':
                queryset = queryset.filter(content_type=reel_ct, object_id__in=Subquery(reel_ids_sub))
            else:
...
class MediaCreateView(APIView):
    ...
    @extend_schema(
        ...
        responses={201: MediaDisplaySerializer},
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            # Additional fields: content_type and object_id must be passed in request data
            ...
            # Optionally check if the content object exists and user has permission to add media
            # For now, we'll assume the user can add media if they own the content object.
            ...
            # Check permission: user must be owner of content object
            if hasattr(content_object, 'user') and content_object.user != request.user:
                return Response(
                    {"error": "You do not have permission to add media to this content"},
                    status=status.HTTP_403_FORBIDDEN
                )
            ...
            return Response(
                MediaDisplaySerializer(media, context={'request': request}).data,
...
class MediaDetailView(APIView):
    ...
    def put(self, request, media_id):
        media = self.get_object(media_id)
        serializer = MediaCreateSerializer(media, data=request.data, partial=True, context={'request': request})
        if serializer.is_valid():
