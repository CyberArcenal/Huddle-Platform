...
class StoryFeedService:
    ...
    def get_story_highlights(
        user: User, days: int = 7, limit: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        highlights = (
            Story.objects.filter(
                user=user, created_at__gte=time_threshold, is_active=True
            )
    ...
    @staticmethod
    def get_story_recommendations(user: User, limit: int = 5) -> List[Dict[str, Any]]:
        """Get story recommendations based on viewing history"""
        # Get users whose stories the current user has viewed
        viewed_stories = ViewService.get_user_viewed(user, limit=100)

        if not viewed_stories:
            # If no viewing history, return popular stories
            return ViewService.get_popular_stories(limit=limit)
        ...
        for creator in list(creators_to_explore)[:limit]:
            creator_stories = StoryService.get_user_stories(
                user=creator, include_expired=False, limit=1  # Get most recent story
            )
